...
class SE_NET(object):
    ...
    def inputs(self, raw_data_batch):
        ...
        speech_f4 = tf.square(tf.math.real(speech_f3)) + tf.square(tf.math.imag(speech_f3))
        ...
        targets = tf.concat(
            [tf.reshape(
                speech_f[i][self.N_IN - 1][0:self.NEFF],
                [1, self.NEFF])
             for i in range(0, self.batch_size)], axis=0)
        ...
        batch_mean, batch_var = tf.compat.v1.nn.moments(images_reshape, [0])
    ...
    def _conv_layer_wrapper(self, input, out_feature_maps, filter_length, is_train):
        '''wrap up all the ops for convolution'''
        filter_width = input.get_shape()[1]
        in_feature_maps = input.get_shape()[-1]
        W_conv = weight_variable([filter_width, filter_length, in_feature_maps, out_feature_maps])
    ...
    def inference(self, images, is_train):
        ...
        # ipdb.set_trace()
        with tf.compat.v1.variable_scope('con1') as scope:
            h_conv1 = self._conv_layer_wrapper(image_input, 12, 13, is_train)
        with tf.compat.v1.variable_scope('con2') as scope:
            h_conv2 = self._conv_layer_wrapper(h_conv1, 16, 11, is_train)
        with tf.compat.v1.variable_scope('con3') as scope:
            h_conv3 = self._conv_layer_wrapper(h_conv2, 20, 9, is_train)
        with tf.compat.v1.variable_scope('con4') as scope:
            h_conv4 = self._conv_layer_wrapper(h_conv3, 24, 7, is_train)
        with tf.compat.v1.variable_scope('con5') as scope:
            h_conv5 = self._conv_layer_wrapper(h_conv4, 32, 7, is_train)
        with tf.compat.v1.variable_scope('con6') as scope:
            h_conv6 = self._conv_layer_wrapper(h_conv5, 24, 7, is_train)
        with tf.compat.v1.variable_scope('con7') as scope:
            h_conv7 = self._conv_layer_wrapper(h_conv6, 20, 9, is_train)
        with tf.compat.v1.variable_scope('con8') as scope:
            h_conv8 = self._conv_layer_wrapper(h_conv7, 16, 11, is_train)
        with tf.compat.v1.variable_scope('con9') as scope:
            h_conv9 = self._conv_layer_wrapper(h_conv8, 12, 13, is_train)
        with tf.compat.v1.variable_scope('con10') as scope:
            f_w = h_conv9.get_shape()[1]
            i_fm = h_conv9.get_shape()[-1]
            W_con10 = weight_variable([f_w, 129, i_fm, 1])
