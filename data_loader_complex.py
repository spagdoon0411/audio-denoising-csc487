...
from models.unet.unet_wrapped import STFTLayer, ISTFTLayer
...
def compose_preprocessing_steps(vec):
    complex_spect = stft(vec)
    real_component = tf.math.real(complex_spect)
    imag_component = tf.math.imag(complex_spect)
    return tf.stack([real_component, imag_component], -1)
